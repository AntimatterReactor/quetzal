...
pub fn decode(input: &[u8]) -> Result<String, ()> {
    ...
    let (mut output, input) = if let Some(i) = input.iter().rev().position(|&x| x == DELIMITER) {
        (
            input
                .get(0..i)
                .ok_or(())?
...
fn test_decode() {
    for t in TESTS {
        assert_eq!(decode(t.1.as_bytes()), Ok(t.0.into()));
    }
}
...
#[test]
fn test_fail_decode() {
    assert_eq!(decode("bcher-kva.ch".as_bytes()), Err(()));
    assert_eq!(decode("+".as_bytes()), Err(()));
    assert_eq!(decode("\\".as_bytes()), Err(()));
    assert_eq!(decode("é".as_bytes()), Err(()));
    assert_eq!(decode("99999999".as_bytes()), Err(()));
}
